...
// ==========================================
const DB_KEY = 'liferpg_nexus_state_v4';

const defaultState = {
    user: { name: "Алекс", globalLevel: 1, xp: 0, xpMax: 100, coins: 0 },
    ...
    history: [
        { id: 1, dateStr: "Системный лог", items: [
            { type: "action", skill: "Инициализация", icon: "fa-power-off", time: "00:00", mins: 0, xp: 0, theme: "color-blue" }
        ]}
    ],
    activeTab: 'activity'
};
...
function commitAction() {
    ...
    if (state.user.xp >= state.user.xpMax) {
        ...
        state.user.xpMax = state.user.globalLevel * 100; // Усложнение
        state.user.coins += 200; // Эмиссия валюты
        alert(`⚡ LEVEL UP!\nДостигнут Lvl ${state.user.globalLevel}. Получено +200 монет!`);
    }
    ...
    feedback.innerHTML = `Начислено: <strong>+${earnedXP} XP</strong> | Прогресс сохранен.`;
...
function buyReward(itemName, cost) {
    if (state.user.coins >= cost) {
        ...
        // СОХРАНЕНИЕ ПАМЯТИ
        saveState();
        alert(`✅ Транзакция успешна! Куплено: ${itemName}.`);
        render();
    } else {
        alert("❌ Недостаточно монет. Возвращайся к работе.");
    }
}
...
function renderActivity() {
    ...
    return `
        ...
        <div class="hint-text">${formatTime(state.currentInput.mins)} | Начислим XP</div>
...
function renderShop() {
    return `
        <div class="shop-header"><h1>Магазин Наград</h1><div class="coin-badge"><span>${state.user.coins}</span><i class="fa-brands fa-ethereum coin-icon"></i></div></div>
        <div class="shop-grid">
            ...
            <div class="shop-item-card">
                ...
                <div class="item-desc">Набор десертов.</div>
            ...
            <div class="shop-item-card full-width">
                ...
                <div style="flex:1">
                    <div class="item-title">ОТДЫХ</div><div class="item-desc">День без рутины.</div>
...
function renderHistory() {
    ...
    state.history.forEach(group => {
        ...
        group.items.forEach(item => {
            if(item.type === 'action') {
                html += `<div class="history-card ${item.theme}"><div class="history-top"><div class="history-title-wrap"><div class="history-icon" style="color:var(--primary-blue)"><i class="fa-solid ${item.icon}"></i></div><div><div class="history-title">${item.skill}</div><div class="history-desc">Выполнение</div></div></div><div class="history-time">${item.time}</div></div><div class="history-bottom"><span>Затрачено: <span class="history-val">${item.mins} минут</span></span><span class="history-gain gain-xp">+${item.xp} XP</span></div></div>`;
            } else {
